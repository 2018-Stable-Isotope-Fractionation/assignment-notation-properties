...
Notes: 
 ...
 - the inline latex equations should be rendered when you rest your mouse over them

## 1A: R and F

### I

Given a sample x with a $\delta^{34}S$ value of -35.5‰ (relative to the sulfur standard Canon Diablo Troilite (CDT)) and $\delta^{15}N$ value of +3.8‰ (relative to the standard Air), calculate $^{34}R$, $^{15}R$, $^{34}F$ and $^{15}F$. You can search the relevant information about the standards online or use the values provided in `isotopia`. Store the resulting values in variables `ratio_34`, `ab_34`, `ratio_15` and `ab_15`, for the ratios and fractional abundances respectively. 
...
In the old days before introduction of suitable standards, isotope ratios were often reported as absolute ratios of the light / heavy isotope. You encounter a 32S/34S ratio of 22.57 in an old paper. Given that the absolute ratio of the modern sulfur standard Canon Diablo Troilite (CDT) is known, what is the $\delta^{34}S$ (vs. CDT) of this old sample? Store the resulting value (in permil) in variable `delta_34`.
...
Calculate $\alpha_{A/B}$, $\epsilon_{A/B}$, $\ln \alpha_{A/B}$, and $\Delta_{A/B}$. Do all calculations within a data frame and store the result in variable `frac_data`. The calculated columns should be named `alpha_AB`, `esp_AB`, `ln_alpha_AB` and `Delta_AB`. Hint: the function to calculate a natural log (ln) is `log()`.
...
Plot `esp_AB`, `ln_alpha_AB` and `Delta_AB` (the ys) vs. `alpha_AB` (the x). Color the lines by the name of each y. Store the resulting plot in variable `p_2B`. Hint: use dplyr's `gather()` function to get the multiple ys into a format that's easy to work with using `ggplot` aesthetics.
...
# 3. Isotope properties

## 3A: Isotopologues

Water has several different stable isotopologues due to the existence of 2 stable isotope of hydrogen and 3 stable isotopes of oxygen. What are the 3 isotopologues of water with the same mass? 
...
The boiling point of pure H2O is 100.00 C - would you expect pure D2O to boil at a higher or lower temperature? Why?
